...
class TwilioController @Inject() (
  ...
  def updateClaimModel(userID: UUID, claimID: UUID, claimSubmissionID: UUID, faxStatus: String): Future[Claim] = {
    claimDAO.findClaim(userID, claimID).flatMap {
      case Some(claim) =>
        val claimState = if (faxStatus == "delivered") Claim.State.SUBMITTED else Claim.State.SUBMISSION_ERROR
        val updatedClaimSubmissions = claim.submissions.map {
          claimSubmission =>
            if (claimSubmission.claimSubmissionID.equals(claimSubmissionID)) {
              claimSubmission.copy(success = faxStatus == "delivered")
            } else {
              claimSubmission
            }
        }

        val newClaim = claim.copy(state = claimState, submissions = updatedClaimSubmissions)
        ...
        params.get("FaxStatus") match {
          case Some(faxStatus) =>
            updateFaxModel(faxStatus, faxId).flatMap {
              (twilioFax: TwilioFax) =>
                updateClaimModel(twilioFax.userID, twilioFax.claimID, twilioFax.claimSubmissionID, faxStatus).map {
                  (claim: Claim) =>
                    (twilioFax, claim)
                }
            }
