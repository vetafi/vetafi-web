...
export interface TosParagraph {
    header: String
    paragraphs: Array<String>
}

const termsOfServiceText: Array<TosParagraph> = [
    {
...
const tosModalTemplate = `
<div id="tos-view" class="page-form">
  <div id="tos-wrapper">
    <h2 class="vfi-title">Terms of Service</h2>
    <div class="vfi-tos-scroller">
      <div *ngFor="let section of termsOfService">
        <h5>{{section.header}}</h5>
        <p *ngFor="let paragraph of section.paragraphs">{{paragraph}}</p>
      </div>
    </div>
    <div class="vfi-checkbox">
      <input type="checkbox" name="accept" [(ngModel)]="accept"/>I have read this Agreement and agree to the terms and conditions.
    </div>
    <div class="vfi-error-msg"></div>
    <div class="button-wrapper">
      <button class="vfi-decline-btn" (click)="activeModal.dismiss()">BACK</button>
      <button class="vfi-accept-btn" (click)="activeModal.close()">ACCEPT</button>
...
@Component({
    selector: 'tos-modal',
    template: tosModalTemplate,
    styleUrls: ['../assets/styles/tos.styl']
})
export class TosModalComponent {
    termsOfService: Array<TosParagraph> = termsOfServiceText;
    accept;
