...
<div id="faq-view">
  <div class="search-wrapper">
    ...
    <input class="search-bar" [(ngModel)]="searchText" (change)="onSearchChange()" placeholder="Type in some keywords to search for content"/>
    <button class="cancel-icon" (click)="cancelSearch()" *ngIf="searchText.length > 0"></button>
  </div>
  <div class="section-card" *ngFor="let section of searchedSections">
    <h3 class="section-title">{{section.title}}</h3>
    <div class="section-divider"></div>
    <div class="section-content" *ngIf="section.html" [innerHTML]="section.html"></div>
    <div class="section-content" *ngIf="section.content">{{section.content}}</div>
  </div>
  <!-- Error Message if search results are empty-->
  <div class="section-card vfi-error" *ngIf="searchedSections.length == 0">
    <div class="warning-icon"></div>
    <div class="section-content">Looks like we can't find what you searched for.<br/>Try simplifying your search keywords to get more results.</div>
  </div>
</div>
...
@Component({
    selector: 'app-faq',
    template: template,
    styleUrls: ['../assets/styles/faq.styl']
})
export class FaqComponent implements OnInit {
    ...
    ngOnInit(): void {
        this.searchedSections = this.sections;
    }

    onSearchChange() {
        this.searchedSections = this.sections.filter((section: any) => {
            return section.content.indexOf(this.searchText) > -1 || isEmpty(this.searchText);
        });
    }
}
